...
export default function ResultPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('regret_analysis_result');
      if (stored) {
  ...
  // Fallback structure for tarot and remedies if not present in stored data
  const tarotData = result.tarot || {
    past_card: {
      number: "V",
      name: "Five of Cups",
      meaning: "Loss & Attachment",
      symbol: "🌊🍷",
      element: "Water",
      tied_to_user: `Reflects past habits of dwelling on decisions like "${result.decision || 'this decision'}".`
    },
    present_card: {
      number: "XVI",
      name: "The Tower",
      meaning: "Upheaval & Truth",
      symbol: "🏰⚡️",
      element: "Fire",
      tied_to_user: "Highlights current anxiety and second-guessing driven by impulse."
    },
    future_card: {
      number: "XVII",
      name: "The Star",
      ...
      symbol: "✨🌌",
      element: "Air",
      tied_to_user: "Predicts rational peace once you allow a cooling period before acting."
    },
    closing_reflection: "True power comes from releasing the fear of potential regret."
  };

  const remedies = result.remedies || [
    {
      step: 1,
      title: "Initiate a 48-Hour Cooling Period",
      description: "Avoid making immediate impulse corrections. Allow emotional intensity to subside before taking further action."
    },
    {
      step: 2,
      title: "Evaluate Irreversible vs. Reversible Factors",
      description: "Separate outcomes you can fix or amend from those that are unchangeable to focus your energy effectively."
    },
    {
      step: 3,
      title: "Establish a Remedial Protocol",
      description: "Draft a clear, single-step contingency plan to mitigate risks identified during this analysis."
    }
  ];
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[#2A2634] pb-6">
            <div>
              ...
              <div className="text-4xl font-black text-[#FF4D4D] font-mono">
                {result.regret_score} <span className="text-xl text-[#94A3B8]">/ 100</span>
              </div>
            </div>
